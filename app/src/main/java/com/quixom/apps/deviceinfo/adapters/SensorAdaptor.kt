...
class SensorAdaptor(internal var c: MainActivity, internal var sensorList: ArrayList<SensorDATA>) : RecyclerView.Adapter<SensorAdaptor.SensorVH>() {
    ...
    class SensorVH(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bindData(sensorInfo: SensorDATA, c: MainActivity) {
            ...
            if (sensorInfo.sensorType == Sensor.TYPE_ACCELEROMETER ||
                    sensorInfo.sensorType == Sensor.TYPE_ACCELEROMETER_UNCALIBRATED ||
            ...
            } else if (sensorInfo.sensorType == Sensor.TYPE_STEP_COUNTER ||
                    sensorInfo.sensorType == Sensor.TYPE_STEP_DETECTOR) {
                // 步数
                ivSensorImage?.setImageResource(R.drawable.ic_if_running_172541)
            } else if (sensorInfo.sensorType == Sensor.TYPE_ROTATION_VECTOR ||
                    sensorInfo.sensorType == Sensor.TYPE_GEOMAGNETIC_ROTATION_VECTOR ||
            ...
            } else if (sensorInfo.sensorType == Sensor.TYPE_GYROSCOPE ||
                    sensorInfo.sensorType == Sensor.TYPE_GYROSCOPE_UNCALIBRATED) {
                // 陀螺仪
            ...
            } else if (sensorInfo.sensorType == Sensor.TYPE_HEART_BEAT ||
                    sensorInfo.sensorType == Sensor.TYPE_HEART_RATE) {
                // 心率
